...
sig_up <- subset(resSig, log2FoldChange > 0.3219)
sig_down <- subset(resSig, log2FoldChange < -0.3219)
...
write.csv(go_bp, file = "a2d1_genes_up_GOBP.csv")
```
```{r GO biological process}
# Map SYMBOL → ENTREZ
mapped <- AnnotationDbi::select(
...
write.csv(go_bp, file = "a2d1_genes_down_GOBP.csv")
```
...
write.csv(go_mf, file = "a2d1_genes_up_GOMF.csv")
```
...
write.csv(go_mf, file = "a2d1_genes_down_GOMF.csv")
```
